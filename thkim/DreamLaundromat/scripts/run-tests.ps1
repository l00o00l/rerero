...
function Invoke-UnityTests {
    ...
    $resultPath = Join-Path $LogDir "dreamlaundromat-$platformName-results.xml"
    $logPath = Join-Path $LogDir "dreamlaundromat-$platformName-tests.log"
    ...
    Write-Host "Running DreamLaundromat $TestPlatform tests..."
    ...
    if (-not (Test-Path $resultPath)) {
        throw "Unity $TestPlatform tests did not produce a result XML. ExitCode=$($process.ExitCode). See $logPath"
    }
    ...
    if ($process.ExitCode -ne 0) {
        throw "Unity $TestPlatform tests passed in XML but Unity exited with code $($process.ExitCode). See $logPath"
    }

    Write-Host "DreamLaundromat $TestPlatform tests passed. Total=$total Passed=$passed Failed=$failed Skipped=$skipped"
}
